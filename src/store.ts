...
export const useDashboardStore = create(persist<Store>((set, get) => ({
  isDrawerOpen: false,
  account: null,
  ...
  setJettons: (balances: JettonsBalancesWithTimestamp["balances"]) => set({
    jettons: {
      balances,
      timestamp: Date.now()
    }
  ...
  normalizedJettons: () => get()?.jettons.balances.map(item => {
