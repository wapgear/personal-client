...
const getScript = (
  src: string,
  setStatus: Dispatch<SetStateAction<ScriptStatus>>
): HTMLElementTagNameMap["script"] => {
  // Fetch existing script element by src
  // It may have been added by another instance of this hook
  const existingScript: HTMLElementTagNameMap["script"] | null =
  ...
  if (existingScript) {
    ...
    setStatus(
      status && Object.values<string>(ScriptStatus).includes(status)
